...
class GetTransferByTokenPort
@Inject
constructor(
    private val fileCacheDataSource: FileCacheDataSource,
    private val transferFileCacheDataSource: TransferFileCacheDataSource
) {
    suspend fun getTransferByTokenPort(tokenPort: String): Flow<DataState<List<HistoryModel>>> =
        flow {
            emit(DataState.Loading())
            val historys = mutableListOf<HistoryModel>()
            val transferFile = transferFileCacheDataSource.getTransferByTokenPort(tokenPort)
